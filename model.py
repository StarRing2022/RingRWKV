...
class RWKV(MyModule):
    ...
    def forward(self, tokens, state, textemd=None, visualemb=None, full_output=False):
        with torch.no_grad():
            w = self.w
            args = self.args
            ...
            if state == None:
                ...
                for i in range(args.n_layer): # state: 0=att_xx 1=att_aa 2=att_bb 3=att_pp 4=ffn_xx
                    ...
                    # 将textemd和visualemb载入
                    if textemd==None and visualemb==None:
                    ...
                    elif textemd!=None and visualemb==None:
                        state[i*5+1] = textemd
                        state[i*5+2] = torch.zeros(args.n_embd, dtype=torch.float, requires_grad=False, device=dev).contiguous()
                    elif textemd!=None and visualemb!=None:
                        state[i*5+1] = textemd
                        state[i*5+2] = visualemb

                        #print(state[i*5+1]) # 文本嵌入式编码
                        #print(state[i*5+2]) # 图像嵌入式编码

                    state[i*5+3] = torch.zeros(args.n_embd, dtype=torch.float, requires_grad=False, device=dev).contiguous() - 1e30
                    state[i*5+4] = torch.zeros(args.n_embd, dtype=atype, requires_grad=False, device=dev).contiguous()

            # print(args.n_layer) #24（1.5B）
            # print(args.n_embd) #2048（1.5B）

            #print(state[0].shape) #[2048]（1.5B）
           

            seq_mode = len(tokens) > 1

            x = w['emb.weight'][tokens if seq_mode else tokens[0]]
            #print(x.shape) #[16,2048]（1.5B）
            ...
            for i in range(args.n_layer):
                ...
                # 将textemd和visualemb载入
                # 将比上述的特征融合更为彻底，但需要重新训练，也就是说，不能使用预训练模型，否则会乱输出
                if textemd==None and visualemb==None:
                    state[i*5+1] = state[i*5+1]
                    state[i*5+2] = state[i*5+2]
                elif textemd!=None and visualemb==None:
                    pass
                    #state[i*5+1] = textemd
                    #state[i*5+2] = state[i*5+2]
                    ...
                    # state[i*5+2] = visualemb
                    #法2
                    # state[i*5+1] = textemd+state[i*5+1] 
                    # state[i*5+2] = visualemb+state[i*5+2]
            ...
            #print(x.shape) #65536，词表大小
            #print(len(state)) #120，隐藏层列表
